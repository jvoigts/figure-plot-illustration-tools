function varargout=plotbar(x_all,y_all,color,varargin)
...
for i=1:size(y_all,2)
    ...
    if size(y_hilo,1)==1 % just plot
        if stripes
            plot(x+([-1 1].*width),y_hilo.*[1 1],'k--','color',color,'LineWidth',3);
        else
            plot(x+([-1 1].*width),y_hilo.*[1 1],'k','color',color,'LineWidth',3);
        end
        
        if numel(extrasymbol)>0
            plot(x,y_hilo,extrasymbol);
        end;
    else
        ...
        if size(y_hilo,1)==2
            ...
            if stripes
                patch(px,py,1,'FaceColor',color,'EdgeColor',color);
            else
                patch(px,py,1,'FaceColor',color,'EdgeColor','none');
            end;
            
            if numel(extrasymbol)>0
                plot(x,mean(y_hilo),extrasymbol,'color',color);
            end;
        ...
        if size(y_hilo,1)==3 % also draw mean
            ...
            if stripes
                patch(px,py,1,'FaceColor','none','EdgeColor',color);
            else
                patch(px,py,1,'FaceColor',color,'EdgeColor','none');
            end;
            ...
            if numel(extrasymbol)>0
                plot(x,mean(y_hilo),extrasymbol,'color',[0 0 0]);
                plot(x+([-1 -.7].*width),y_hilo(2).*[1 1],'k','color',color,'LineWidth',2);
                plot(x+([1 .7].*width),y_hilo(2).*[1 1],'k','color',color,'LineWidth',2);
                
            else
                plot(x+([-1 1].*width),y_hilo(2).*[1 1],'k','color',color,'LineWidth',2);
        ...
        if numel(secondarycolor)>0 % draw a secondary color
            plot(x.*[1 1],y_hilo([1 end]),'k','color',secondarycolor,'LineWidth',2);
